...
router.get('/users/course/:id/', async(req, res) => {
    ...
    const posts = await pool.query("SELECT * FROM posts WHERE course_id = $1", [id]);
    ...
    for(let i = 0; i < posts.rows.length; i++) {
        posts_list[i] = {title: posts.rows[i].title, text: posts.rows[i].text, author: posts.rows[i].username, date: posts.rows[i].date};
    }
